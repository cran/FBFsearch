...
double log_FBF_Ga_Gb(vec G_a, vec G_b, int edge, mat edges, mat YtY, int add, double n, double h)
{
  ...
  vec V1, V2, G1, V11, pa1, pa0, betah, vv(1);
  uvec iw, ipa1;
  mat e, yty, XtX, Xty, invXtX, _sigma;   
  ...
  iwi=conv_to<int>::from(iw); _sigma=invXtX(iwi,iwi); sigma=conv_to<double>::from(_sigma);	
...
RcppExport SEXP FBF_RS(SEXP Corr, SEXP nobs, SEXP G_base, SEXP h, SEXP C, SEXP n_tot_mod, SEXP n_hpp)
{
 ...
 iM_P=conv_to<vec>::from(sort_index(conv_to<vec>::from(M_P), 1));
